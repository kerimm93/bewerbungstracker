...
const context = {
  ...
  copyText(value){ copiedText = String(value); }, toast(){}, updateProgress(){}, promptHTML: v => v,
...
assert.strictEqual(JSON.stringify(normalized.slots.map(s => s.firmId)), JSON.stringify(['a', '', '', 'b']));
assert.strictEqual(normalized.slots[3].stepIndex, 2, 'invalid occupied step must fall back to next step');
...
elements['panel-batch'] = { addEventListener(type, handler){ listeners[type].push(handler); }, _batchBound: false };
elements['batch-cards'] = { style: { props: {}, setProperty(k, v){ this.props[k] = v; } }, addEventListener(){ throw new Error('listeners must bind to panel-batch, not batch-cards'); }, _batchBound: false, innerHTML: '' };
function actionEl(attrs, value){
  return {
    value,
    closest(selector){ return selector === '[data-batch-action]' ? this : null; },
    getAttribute(name){ return attrs[name] == null ? null : String(attrs[name]); }
  };
}
function renderedCardCount(){ return (elements['batch-cards'].innerHTML.match(/<div class="batch-card(?: |")/g) || []).length; }
function renderedEmptyCardCount(){ return (elements['batch-cards'].innerHTML.match(/<div class="batch-card empty"/g) || []).length; }
...
assert.strictEqual(renderedCardCount(), 4);
assert.strictEqual(listeners.click.length, 1, 'batch click handler must be bound once on panel-batch');
context.renderBatchMode();
assert.strictEqual(listeners.click.length, 1, 'rerender must not stack batch click handlers');

context.setBatchSlotCount(2);
listeners.click[0]({ target: actionEl({ 'data-batch-action': 'slot-count', 'data-slot-count': '3' }) });
assert.strictEqual(context.batchUiState.slotCount, 3);
assert.strictEqual(renderedCardCount(), 3);
listeners.click[0]({ target: actionEl({ 'data-batch-action': 'slot-count', 'data-slot-count': '4' }) });
assert.strictEqual(context.batchUiState.slotCount, 4);
assert.strictEqual(renderedCardCount(), 4);

context.handleBatchClick({ target: actionEl({ 'data-batch-action': 'clear', 'data-slot-index': '1' }) });
assert.strictEqual(context.batchUiState.slots[1].firmId, '', 'clear action must keep the target slot empty in state');
assert.strictEqual(renderedEmptyCardCount(), 1, 'cleared slot must render as empty');
context.renderBatchMode();
assert.strictEqual(context.batchUiState.slots[1].firmId, '', 'rerender must not auto-fill cleared slot');
assert.strictEqual(renderedEmptyCardCount(), 1);
context.setBatchSlotCount(2);
context.setBatchSlotCount(4);
assert.strictEqual(context.batchUiState.slots[1].firmId, '', 'cleared slot must survive 4 -> 2 -> 4');
assert.strictEqual(renderedEmptyCardCount(), 1);
context.batchUiState = null;
context.loadBatchUiState();
context.renderBatchMode();
assert.strictEqual(context.batchUiState.slots[1].firmId, '', 'cleared slot must survive localStorage reload');
assert.strictEqual(renderedEmptyCardCount(), 1);
listeners.change[0]({ target: actionEl({ 'data-batch-action': 'firm', 'data-slot-index': '1' }, 'b') });
assert.strictEqual(context.batchUiState.slots[1].firmId, 'b', 'explicit firm selection must fill the empty slot');
assert.strictEqual(renderedEmptyCardCount(), 0);

context.batchUiState = context.normalizeBatchUiState({ slotCount: 2, slots: [{ firmId: 'b', stepIndex: 0 }, { firmId: 'a', stepIndex: 1 }] });
context.renderBatchMode();
assert(elements['batch-cards'].innerHTML.includes('value="0" selected'), 'explicit stepIndex 0 must render as selected even when next step is later');
context.handleBatchClick({ target: actionEl({ 'data-batch-action': 'copy-start', 'data-slot-index': '0' }) });
assert(copiedText.includes('„Eignungsprüfung“'), 'start prompt must use explicit step 0');
context.handleBatchClick({ target: actionEl({ 'data-batch-action': 'copy-handoff', 'data-slot-index': '0' }) });
assert(copiedText.includes('"stepIndex": 0'), 'handoff prompt must use explicit step 0');
